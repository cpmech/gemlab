...
mod tests {
    ...
    fn capture_some_errors() {
        ...
        assert_eq!(
            integ::mat_08_ntn(&mut kk, &mut pad, 1, 0, clear, axis, &[], f).err(),
            Some("nrow(K) must be ≥ ii0 + nnode ⋅ space_ndim")
        );
        assert_eq!(
            integ::mat_08_ntn(&mut kk, &mut pad, 0, 1, clear, axis, &[], f).err(),
            Some("ncol(K) must be ≥ jj0 + nnode ⋅ space_ndim")
        );
        // more errors
        let ips = &IP_LIN_LEGENDRE_1;
        assert_eq!(
            integ::mat_08_ntn(&mut kk, &mut pad, 0, 0, clear, axis, ips, f).err(),
        ...
        assert_eq!(
            integ::mat_08_ntn(&mut kk, &mut pad, 0, 0, clear, axis, ips, |_, _, _, _| Err("stop")).err(),
            Some("stop")
        );
    }
    ...
    fn tri3_works() {
        ...
        selection.iter().zip(tolerances).for_each(|(ips, tol)| {
            ...
            integ::mat_08_ntn(&mut kk, &mut pad, 0, 0, clear, axis, ips, |tt, _, _, _| {
                tt.sym_set(0, 0, rho);
                tt.sym_set(1, 1, rho);
                Ok(1.0)
            })
